...
class RD_loss3(nn.Module):
    ...
    def compute_rd_map_list(self,depth_tensor):
        ...
        rd_list=[rd_top,rd_right,rd_bottom,rd_left,rd_left_top,rd_right_top,rd_bottom_right,rd_bottom_left]
    ...
    def forward(self,pred,gt):
        ...
        for i in range(len(pred_rd_list)):
            # rd=torch.abs(torch.exp(pred_rd_list[i])/torch.exp(gt_rd_list[i])-1)
