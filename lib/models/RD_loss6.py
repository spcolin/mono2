...
class RD_loss6(nn.Module):

    def __init__(self,span=30,sample_count=1500):
        super(RD_loss6, self).__init__()
        self.span=span
        self.range_list=np.random.randint(1,self.span+1,size=8)

        x_pos = np.random.randint(30+self.span,350-self.span,size=sample_count)
        y_pos = np.random.randint(30+self.span,350-self.span,size=sample_count)

        self.point_pos=[[x_pos[i],y_pos[i]] for i in range(sample_count)]
    ...
    def compute_rd(self,depth,pos):
        ...
        rd_top=depth[:,:,pos[1],pos[0]]-depth[:,:,pos[1]-self.range_list[0],pos[0]]
        rd_bottom=depth[:,:,pos[1],pos[0]]-depth[:,:,pos[1]+self.range_list[1],pos[0]]
        rd_left=depth[:,:,pos[1],pos[0]]-depth[:,:,pos[1],pos[0]-self.range_list[2]]
        rd_right=depth[:,:,pos[1],pos[0]]-depth[:,:,pos[1],pos[0]+self.range_list[3]]

        rd_left_top=depth[:,:,pos[1],pos[0]]-depth[:,:,pos[1]-self.range_list[4],pos[0]-self.range_list[4]]
        rd_right_top=depth[:,:,pos[1],pos[0]]-depth[:,:,pos[1]-self.range_list[5],pos[0]+self.range_list[5]]
        rd_left_bottom=depth[:,:,pos[1],pos[0]]-depth[:,:,pos[1]+self.range_list[6],pos[0]-self.range_list[6]]
        rd_right_bottom=depth[:,:,pos[1],pos[0]]-depth[:,:,pos[1]+self.range_list[7],pos[0]+self.range_list[7]]
