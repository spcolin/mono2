...
class Refine_module4(torch.nn.Module):

    def __init__(self, load_pretrained=True, pretrained_resnet18_path=None):
        super(Refine_module4, self).__init__()
    ...
    def forward(self, depth_tensor, img_tensor):
        ...
        resample_input = torch.cat((img_tensor, depth_tensor), 1)
        resample_grid = self.resample_net(resample_input)
        resampled_depth = self.sample(resample_grid, depth_tensor)
