...
class Residual(torch.nn.Module):
    ...
    def forward(self, input):
        feature = self.backbone(input)  # B*2048*13*13

        up1 = F.interpolate(feature, scale_factor=2, mode='bilinear', align_corners=True)  # B*2048*26*26
        ...
        cat1 = torch.cat([up1, F.interpolate(input, size=up1.shape[2:])], 1)  # B*516*52*52
        ...
        cat2 = torch.cat([up2, F.interpolate(input, size=up2.shape[2:])], 1)  # B*68*208*208
...
class Refine_module2(torch.nn.Module):
    ...
    def __init__(self, load_pretrained=True, pretrained_resnet50_path=None):
        ...
        self.residual_net = Residual(load_pretrained, pretrained_resnet50_path)
        self.resample_net = Resample(load_pretrained, pretrained_resnet50_path)
